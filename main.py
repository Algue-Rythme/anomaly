...
def generate_adversarial(model, x_0, margin, phase):
    max_iter = 10
    lr       = 5.
    h_x_0    = 2.
    h_x      = 0.

    learning_rate = lr * (margin / max_iter)
    ...
    for _ in range(max_iter):
        with tf.GradientTape() as tape:
            y = model(x)
            dispersion   = h_x   * metric_entropy(x, x, margin)
            fidelity     = h_x_0 * metric_entropy(x, x_0, margin)
...
def one_class_wasserstein(model, x, optimizer, lbda, alpha, margin, phase):
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        hinge       = oneclass_hinge(1., y) #  + oneclass_hinge(margin, -y_adv)
